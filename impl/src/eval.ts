...
export class Evaluator {
  ...
  private tzCheckLogs: { log: TzCheck[]; baseAlign: GridTag | null }[] = [];
  ...
  private applyingBodies = new Map<Expr, string>();
  ...
  registerPremise(st: Statement & { t: 'premiseDef' }) {
    ...
    if (st.expr) {
      ...
      // 宣言必須「寄り」で未執行＝従来どおり）。
      for (const [k, v] of this.overlayMembers(new Map(), base)) members.set(k, v);
  ...
  resolve(name: string, env: Env): V {
    ...
    if (this.rt.topBindings.has(name)) {
      ...
      if (hit !== undefined) {
        ...
        for (const c of hit.tzChecks) this.checkTzMembership(this.predicateAlign, c.win, c.ctx, true);
      ...
      const tzFrame = { log: [] as TzCheck[], baseAlign: this.predicateAlign };
  ...
  private checkTzMembership(input: GridTag | null | undefined, win: GridTag | null | undefined, ctx: string,
                            fromPredicate = false): void {
    ...
    if (fromPredicate) {
      ...
      for (const f of this.tzCheckLogs) {
        if (this.predicateAlign === f.baseAlign
            && !f.log.some(c => c.ctx === ctx && gridEq(c.win, win))) f.log.push({ win, ctx });
      }
    }
    ...
    if (e.callee.t === 'name') {
      ...
      if (local === undefined) {
        ...
        if (binding && binding.decl.params.length > 0) {
          ...
          try { return this.evalExpr(binding.decl.rhs, childEnv(base, locals)); }
  ...
  applyValue(callee: V, args: V[], env: Env, calleeName?: string): V {
    if (isObj(callee)) {
      if (callee.k === 'lambda') {
        ...
        } finally { this.applyingBodies.delete(callee.body); }
      }
      if (callee.k === 'cycle') {
  ...
  private rollMember(env: Env): Expr {
    ...
    if (!m) this.err('roll は規約が必要（I3）——位置引数で Following/Preceding を書くか前文で roll: を宣言');
