...
describe('ADR-41: isOpen（導出値述語・半開区間の和）', () => {
  // timeout 明示: 単体実行では常に 5 秒未満だが、並列実行の高負荷時に既定 5000ms を跨ぐ偽陽性が
  // 頻発した（実測 5.0〜6.5 秒・2026-08-26。性能退行ではなくスケジューリング起因の既知フレーク）
  it('二部制の真偽: 前場内 真・昼休み 偽・後場内 真・引け後 偽・境界は半開（9:00 真・11:30 偽）', { timeout: 15_000 }, () => {
