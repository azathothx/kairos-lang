...
describe('ADR-41: isOpen（導出値述語・半開区間の和）', () => {
  // 注: このファイルは計算重（hour 級グリッド）——タイムアウトは vitest.config の 30s（CI は 90s）に
  // 任せる。テストは必ず impl/ から実行する（リポジトリルートからだと config が拾われず既定 5s で
  // 偽陽性タイムアウトになる——2026-08-26 に実測・個別 timeout の上書きはかえって網を狭めるので不可）
