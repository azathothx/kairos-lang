...
| `cycle(labels) anchor:` | Attaches repeating labels to a partition window. Labels, not windows. Period length and target are arbitrary (`year cycle` is fine). `anchor:`: the target window it falls in carries the head label. The bound name reads as a point → label value function | §3.6 · `stdlib/gregorian.md` §4 · ADR-27 |
...
| label projection (bound name)（ラベル射影（束縛名）） | The bound name of a labeled window/cycle/table doubles as the projection name (`weekday(d)`, `sekki(d)`). Points store no labels; labels are read via projections (the generic word `labelOf` was retired; ADR-30) | §4.9 · §3.6 |
