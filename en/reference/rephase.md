...
> Translated from the canonical Japanese page [reference/rephase.md](../../reference/rephase.md).
> The `source_sha` above records the source revision; a consistency check flags this page when the
> Japanese original changes.

**Category**: derivation sugar (premise layer) / **Signature**:
`|> rephase(δ, on: W, unit: U) : premise -> premise` / official name (ruled 2026-07-26; formerly the
placeholder `shiftBoundary`; spec §5.4)
...
```text
rephase(δ, on: W, unit: U)  ≡  W = U span (_ => k) phase: ((φ₀ + δ) mod k)   # negative δ is also normalized by the modulus (F65); the base's label: is preserved (F96)
...
- **The scope is pairs with constant `k`** (`year ⊃ month`) only. Pairs where `k` varies (shifting
  ...
  out of scope — if ever needed, a separate operator (open item; the word `boundary` is reserved
  for it).
