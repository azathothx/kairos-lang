...
**分類**: 射影（本体層 §4.9・値関数） ／ **シグネチャ**: `coincides(S, w, d) : 点 -> Bool` ／ 名は**確定**（2026-07-09・F51 の一括確定。比較候補 `hits`/`anyIn`/`sharesWindow` は不採用）
...
除外は `coincides` が「日」の所属で判定する（発火時刻 09:00 は保存される）。なおこの
「tick＋coincides」の型は **stdlib 糖衣 [`at`](at.md) に 1 語で縮む**（`… |> at(T09:00)`＝
展開が本節の正準形そのもの。ADR-51）——以下は展開形の理解のための正準例:
