...
- **anchor の選定（実務ガイダンス・実装系還流 2026-08）**: anchor には**照合済みの実日をそのまま
  書く**のが原則。照合の要否は anchor の性格で分かれる——**観測照合型**（日干支・二十八宿など
  「どの実日がどのラベルか」が一次資料・出版物との照合でしか確定しないもの）と**規約定義型**
  （節切りの「立春の窓が寅月」のように定義そのものが位相を与えるもの＝照合不要）。伝統順のリストと
  照合実日の位相が合わないときは、**照合実日を動かさずリスト側を回転**する（上項の裏面適用——
  「回転は anchor の取り直しとセット」の、照合値を固定する側の選択）。
