# <img src="../assets/logo/kairos-pipe.svg" width="30" height="30" alt=""> 記述語リファレンス
...
各ページのコード例は実行検証済み。例中の `# eval:`／`#=>` の意味はページ下部の
[実行例の規約（doctest）](#実行例の規約doctest)——読むだけなら「`# eval:` が評価範囲・`#=>` が
その実行結果」と分かっていれば足りる。語彙からでなく**要求から**引きたいときは
[レシピ集](../recipes/README.md)（月末営業日・イースター・4-4-5 など・1 要求 1 ページ）。

## 索引

<p align="center"><img src="../assets/figures/operator-map.ja.svg" width="920"
  alt="記述語の全体地図: 本体層の七族（生成子・窓・選択子・点変換・フィルタ・結合子・ストライド）を合成し、射影が値式と接続、premise 層が語彙を供給する。"></p>
...
- ブロック内の `# eval: FROM..TO` 行が評価範囲（`[FROM, TO)`）。任意後置 `tz: Zone`（例
  ...
  端点と表示だけが実行 tz 側になる）。
- `#=>` 行が期待値（最後の本体式の結果。空白区切り・複数行可）。**行が無ければ「点ゼロ（空列）」の
  主張**（`#~>` の既定と対称——空テーブル〈ADR-45〉級の例は `#=>` を書かないことで空を照合する。
  `#=>` 単独行は期待値として読まれないので書かない）。どちらも Kairos の行コメントなので
  ソースとしてそのまま妥当。真夜中ちょうどの点は日付のみで印字される（day 粒度の点と同表記）。
  秒未満の端数は表示では切り捨てられる（`…T00:39:35.244` の点は `…T00:39:35` と印字。点の同一性は
