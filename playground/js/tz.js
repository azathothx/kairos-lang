...
export class Tz {
    ...
    anchor(localAsUtc) {
        ...
        while (lo < hi) { // localAsUtc 近傍の区間添字（±1 日で足りる）
            const mid = (lo + hi + 1) >> 1;
            if (this.t[mid] <= localAsUtc - DAY_MS)
                lo = mid;
            else
                hi = mid - 1;
        }
        for (let i = lo; i < this.t.length && this.t[i] <= localAsUtc + DAY_MS; i++) {
            ...
            if (this.offsetAt(u) === o)
                cands.push(u);
        }
        cands.sort((a, b) => a - b);
        if (cands.length === 1)
            return { kind: 'unique', ms: cands[0] };
        if (cands.length >= 2)
            return { kind: 'overlap', ms: cands[0], alt: cands[cands.length - 1] };
        ...
        for (let i = 1; i < this.t.length; i++) {
            ...
            if (before < localAsUtc && localAsUtc <= after)
                return { kind: 'gap', ms: tr };
        }
    ...
    dayStarts(hi) {
        if (hi > this.daysHi) {
            if (this.days.length === 0)
                this.days.push(this.civilDayStart(1970, 1, 1));
    ...
    dayIndex(dayStartMs) {
        ...
        while (days[i] > dayStartMs && i > 0)
            i--;
        while (i + 1 < days.length && days[i + 1] <= dayStartMs)
            i++;
        return i;
    }
