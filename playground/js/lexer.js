...
const isNamePart = (c) => isLetter(c) || isDigit(c);
export function lex(src) {
    ...
    const err = (msg) => { throw new LexError(msg, line, col); };
    while (i < src.length) {
        const c = src[i];
        if (c === '#') { // 行コメント
        ...
        if (c === '"') {
            ...
            while (j < src.length && src[j] !== '"' && src[j] !== '\n')
                j++;
            if (src[j] !== '"')
                err('文字列リテラルが閉じていない（改行は含められない。ADR-32）');
        ...
        if (isDigit(c)) {
            ...
            if (w && !/^\d+(?:\.\d+)?(?:[eE]|$|[^dhms\d.])/.test(src.slice(i))) {
                ...
                if (hasCivil && hasElapsed)
                    err(`市民時と経過時間の幅は混合できない: ${text}（ADR-28）`);
                const width = hasCivil ? { kind: 'civil', days: civil } : { kind: 'elapsed', ms: elapsed };
        ...
        if (c === 'T' && /^T\d{2}:/.test(src.slice(i, i + 4))) {
            ...
            if (!m)
                err('単独時刻リテラルの形は Thh:mm（例 T07:00・秒まで書くなら Thh:mm:ss。ADR-51）');
        ...
        if (isLetter(c)) {
            let j = i;
            while (j < src.length && isNamePart(src[j]))
                j++;
            let text = src.slice(i, j);
            if (text === 'calendar' && src.slice(j, j + 7) === '-system') {
        ...
        if (c === '¥' || c === '¥')
            err('円記号 ¥（U+00A5）は差演算子ではない。バックスラッシュ U+005C を使う（§4.5）');
