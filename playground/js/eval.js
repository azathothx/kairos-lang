...
import { parseCoveringText } from './parser.js';
...
export class OutOfCoverageSignal extends KairosError {
    ...
    window;
    constructor(msg, entries, window) {
        super(msg);
 ...
 *  1 日ずれは起き得ない） */
const VACUOUS_GRAIN = { kind: 'vacuous', step: 0, phase: 0, off: 0, tz: '' };
const isVacuous = (a) => a?.kind === 'vacuous';
function gridEq(a, b) {
    if (isVacuous(a) || isVacuous(b))
        return true; // 空虚適合は相手が「なし」でも通す（ADR-45）
    return !!a && !!b && a.kind === b.kind && a.step === b.step && a.phase === b.phase
        && a.off === b.off && a.tz === b.tz;
}
const gridDesc = (a) => isVacuous(a) ? '空虚適合（空テーブル）'
...
const isValueV = (v) => typeof v === 'number' || typeof v === 'string' || typeof v === 'boolean';
const kindOf = (v) => Array.isArray(v) ? 'list' : typeof v === 'object' && v !== null ? v.k : typeof v;
export class PremiseInstance {
    name;
    members;
    defs;
    base;
    constructor(name, members, defs, base) {
        this.name = name;
...
export class Runtime {
    ...
    coverage = new Map();
    vocab = new Set(['Following', 'Preceding', 'Modified', 'latest',
        'clip', 'drop', 'error', 'keep']);
    tzName; // 実行既定 tz（CLI/API 引数。前文 tz: が在圏で上書きする）
    tz; // 実行既定 tz の幾何（表示・from/to 解決）
    epoch; // 1970-01-01T00:00（実行既定 tz）の UTC ms＝実体化の下限
    computeEnd;
    fromMs;
    toMs;
    constructor(fromMs, toMs, tz) {
        this.fromMs = fromMs;
        this.toMs = toMs;
        this.tzName = tz;
        this.tz = getTz(tz);
        ...
        if (fromMs < this.epoch)
            throw new KairosError('プロトタイプの評価範囲は 1970-01-01 以降');
    }
...
/** 全時間 (-∞, +∞) に対する区間列の補集合 */
function complementIv(iv) {
    const m = mergeIv(iv);
    const out = [];
    let cur = -Infinity;
    for (const x of m) {
        if (x.start > cur)
            out.push({ start: cur, end: x.start });
        cur = Math.max(cur, x.end);
    }
    if (cur < Infinity)
        out.push({ start: cur, end: Infinity });
    return out;
}
function ivContainsPoint(iv, ms) {
    return iv.some(x => x.start <= ms && ms < x.end);
}
/** c ⊆ ∪iv か（被覆主張の必要条件検査・ADR-37 判断 5） */
function ivCovers(iv, c) {
    return mergeIv(iv).some(x => x.start <= c.start && c.end <= x.end);
}
const annKey = (a) => `${a.source}#${a.covering}#${a.asof ?? ''}`;
/** 同一属性（源・covering・asof）の註釈区間を整列・併合した正規形 */
function normAnn(list) {
...
function rollImageAnn(ann, axis, conv) {
    if (axis.length === 0)
        return normAnn(ann);
...
function dependImageAnn(axisAnn, axis, conv) {
...
function shiftAnnByWindows(ann, iv, n) {
    const moveIn = (ms, i) => {
        const j = i + n;
        if (j < 0)
            return -Infinity;
...
function shiftAnnByAxis(ann, axis, n) {
    if (axis.length === 0)
        return normAnn(ann);
    const step = (k) => (k < 0 ? -Infinity : k >= axis.length ? Infinity : axis[k]);
    return normAnn(ann.map(a => {
...
function isExternalCall(x) {
    return !!x && typeof x === 'object' && x.t === 'call'
        && x.callee?.t === 'name' && x.callee.name === 'external';
}
export class Evaluator {
     ...
     *  baseAlign＝束縛評価開始時の predicateAlign（記録条件②の基準。還流第 17 便 §2） */
    tzCheckLogs = [];
    ...
    severedLocalsEnv = null;
    rt;
    constructor(rt) { this.rt = rt; }
    // ---------- プログラム ----------
    runProgram(program, defaultMembers) {
        const results = [];
        let env = { rt: this.rt, premise: null, members: defaultMembers, locals: null, parent: null };
        const execStatements = (stmts, env0) => {
            let cur = env0;
                ...
                switch (st.t) {
                    ...
                    case 'preamble': {
                        ...
                        if (st.name) {
                            ...
                            for (const [k, v] of premise.members)
                                members.set(k, v);
                        }
                        ...
                        for (const m of st.members)
                            members.set(m.key, m.value);
                        const next = { rt: this.rt, premise, members, locals: null, parent: null };
                        if (st.block) {
                            execStatements(st.block, next);
                        }
                        else
                            cur = next; // 次の前文まで以降の文を統べる（§3.2）
                        break;
                    }
                    case 'binding': {
                        if (CORE_WORDS.has(st.name))
                            this.err(`core 語 ${st.name} の再定義は静的エラー（§4.8）`);
                    ...
                    case 'streamExpr': {
                        ...
                        const disp = (ms) => this.rt.fmt(ms % 1000 === 1 ? ms - 1 : ms);
                        const annotations = clipAnn(s.ann, this.rt.fromMs, this.rt.toMs).map(a => ({
    ...
    registerPremise(st) {
        ...
        const absorb = (block) => {
            ...
            for (const b of block.bindings) {
                if (CORE_WORDS.has(b.name))
                    this.err(`core 語 ${b.name} の再定義は静的エラー（§4.8）`);
        ...
        if (st.expr) {
            ...
            for (const stage of st.expr.stages) {
                if (stage.name !== 'rephase')
                    this.err(`premise 段 ${stage.name} は未対応（プロトタイプ）`);
        ...
        if (epochE !== undefined) {
            const env = { rt: this.rt, premise: null, members: new Map(), locals: null, parent: null };
    ...
    rephaseArgs(stage) {
        ...
        for (const a of stage.args) {
            if (!a.name)
                d = this.constNum(a.value);
            else if (a.name === 'on')
                W = a.value.t === 'name' ? a.value.name : undefined;
            else if (a.name === 'unit')
                U = a.value.t === 'name' ? a.value.name : undefined;
        }
        if (d === undefined || !W || !U)
            this.err('rephase(δ, on: W, unit: U) の引数が不足');
        return { d: d, W: W, U: U };
    }
    expandShiftBoundary(base, d, W, U) {
        ...
        const named = { phase: { t: 'num', v: ((phi0 + d) % k + k) % k } };
        if (g.named.label)
            named.label = g.named.label;
        return {
            params: [],
            rhs: {
    ...
    constNum(e) {
        if (e.t === 'num')
            return e.v;
        if (e.t === 'neg')
            return -this.constNum(e.e);
        this.err('数値定数を期待');
    }
    constLambdaValue(e) {
        if (e.t === 'lambda' && e.body.t === 'num')
            return e.body.v;
    ...
    hasDateTable(e) {
        ...
        if (e.t === 'list') {
            if (e.covering)
                return true;
            if (e.elems.some(el => ('t' in el) && (el.t === 'date' || el.t === 'range')))
                return true;
        }
        for (const v of Object.values(e)) {
            if (Array.isArray(v)) {
                if (v.some(x => x && typeof x === 'object' && this.hasDateTable(x)))
                    return true;
            }
            else if (v && typeof v === 'object' && 't' in v && this.hasDateTable(v))
                return true;
        }
        return false;
    }
    ...
    scanVocab(e) {
        const visit = (x) => {
            ...
            if (x.t === 'gen' && x.word === 'cycle' && x.arg.t === 'list') {
                for (const el of x.arg.elems)
                    if ('t' in el && el.t === 'name')
                        this.rt.vocab.add(el.name);
            }
            if (x.t === 'list' && x.labels && x.labels.t === 'list') {
                for (const el of x.labels.elems)
                    if ('t' in el && el.t === 'name')
                        this.rt.vocab.add(el.name);
            }
            // segmentBy の labels: 引数（ADR-39）の列挙名も語彙に収集（named-arg の形で現れる）
            if (!('t' in x) && x.name === 'labels' && x.value?.t === 'list') {
                for (const el of x.value.elems) {
                    if ('t' in el && el.t === 'name')
                        this.rt.vocab.add(el.name);
                }
            }
            // labels: cycle 形（ADR-47）の列挙名も同様に収集
            if (!('t' in x) && x.name === 'labels' && x.value?.t === 'cycleLabels'
                && x.value.list?.t === 'list') {
                for (const el of x.value.list.elems) {
                    if ('t' in el && el.t === 'name')
                        this.rt.vocab.add(el.name);
                }
            }
    ...
    resolve(name, env) {
        ...
        if (env.premise) {
            ...
            if (def)
                return this.evalDef(env.premise, name, def, env);
            const cs = this.calendarSystemOf(env);
            if (cs) {
                const def2 = cs.findDef(name);
                if (def2)
                    return this.evalDef(cs, name, def2, env);
            }
        }
        else {
            const cs = this.calendarSystemOf(env);
            if (cs) {
                ...
                if (def)
                    return this.evalDef(cs, name, def, env);
            }
        }
        ...
        if (this.rt.topBindings.has(name)) {
            ...
            const rhsEnv = { ...env, locals: null, parent: null };
            const tzFrame = { log: [], baseAlign: this.predicateAlign };
            this.tzCheckLogs.push(tzFrame);
            let v;
            try {
                v = this.evalExpr(b.rhs, rhsEnv);
            }
            finally {
                this.externalCtx = prevCtx;
                this.severedLocalsEnv = prevSevered;
                this.resolvingDefs.pop();
                this.tzCheckLogs.pop();
            }
            if (isObj(v) && v.k === 'table' && !v.src)
                v = { ...v, src: name }; // 出自の焼印（ADR-37 判断 2）
            if (isObj(v) && v.k === 'windows' && !v.name)
                v = { ...v, name }; // 診断用の束縛名（ADR-42）
            if (b.covering)
                v = this.applyClaim(v, b.covering, name, env); // 明示の被覆主張（判断 5）
            this.defCache.set(key, { v, tzChecks: tzFrame.log });
            return v;
        }
        if (name === 'chronos')
            return { k: 'chronos' };
        if (name === 'everyDay')
            return this.everyDay(env);
        if (name === 'everyInstant')
            return { k: 'instant' };
        if (this.rt.vocab.has(name))
            return name; // 列挙ラベル（意味論で区別。§5.6 注記）
    ...
    isBoundName(name, env) {
        if (this.lookupLocal(name, env) !== undefined)
            return true;
        if (env.premise?.findDef(name))
            return true;
        const cs = this.calendarSystemOf(env);
        if (cs?.findDef(name))
            return true;
    ...
    overlayMembers(base, root) {
        const chain = [];
        for (let p = root; p; p = p.base)
            chain.unshift(p);
        if (chain.every(p => p.members.size === 0))
            return base;
        const m = new Map(base);
        for (const p of chain)
            for (const [k, v] of p.members)
                m.set(k, v);
        return m;
    }
    memberStr(members, key) {
    ...
    evalDef(root, name, def, env) {
        ...
        const defEnv = { ...env, premise: root, members, locals: null, parent: null };
        ...
        let v;
        try {
            v = this.evalExpr(def.rhs, defEnv);
        }
        finally {
            this.externalCtx = prevCtx;
            this.severedLocalsEnv = prevSevered;
        ...
        if (isObj(v) && v.k === 'windows' && !v.name)
            v = { ...v, name: `${root.name}.${name}` }; // 診断用の束縛名（ADR-42）
        if (def.covering)
            v = this.applyClaim(v, def.covering, `${root.name}.${name}`, defEnv); // 被覆主張
        this.defCache.set(key, { v, tzChecks: tzFrame.log });
        return v;
    }
    ...
    applyClaim(v, claim, source, env) {
        if (isObj(v) && v.k === 'lambda')
            this.err(`被覆主張は引数なしの束縛にのみ付く: ${source}（ADR-37）`);
        ...
        const ann = complementIv(iv).map(x => ({
    ...
    checkExternalPositions(root, name, def) {
        const legal = new Set();
        if (isExternalCall(def.rhs))
            legal.add(def.rhs);
        if (def.rhs.t === 'pipe' && isExternalCall(def.rhs.head))
            legal.add(def.rhs.head);
        let found = false;
        const walk = (x) => {
            if (!x || typeof x !== 'object')
                return;
    ...
    evalExternal(e, env) {
        ...
        for (const a of e.args) {
            if (!a.name)
                this.err('external は named-arg のみを取る（kind: が必須。ADR-46）');
            if (a.name === 'kind') {
                ...
                if (a.value.t !== 'list')
                    this.err('external の labels: はラベル値域の列挙リスト（ADR-46）');
                declLabels = a.value.elems.map(el => ('t' in el && el.t === 'name') ? el.name : this.err('external の labels: の要素はラベル名'));
                if (declLabels.length === 0)
                    this.err('external の labels: は空にできない（無宣言＝ラベルなし。ADR-46）');
            }
            else if (a.name === 'source') {
                const v = this.evalExpr(a.value, env);
                if (typeof v !== 'string')
                    this.err(`external の source: は文字列リテラル: ${kindOf(v)}`);
                srcOverride = v;
            }
            else {
        ...
        if (!kind)
            this.err('external は kind: が必須（dates | instants＝整列の主張。ADR-46）');
        ...
        if (data === undefined) {
            const decl = { kind, ...(declLabels ? { labels: declLabels } : {}), source };
            if (!this.rt.resolver) {
    ...
    externalToTable(owner, name, kind, declLabels, data, env) {
        ...
        try {
            ...
            this.err(`契約違反: covering が読めない——external ${src}: ${err.message}`);
        }
        ...
        if (kind === 'dates') {
            ...
            if (data.instants)
                this.err(`契約違反: kind: dates に instants が来た——external ${src}`);
            ...
            if (data.dates)
                this.err(`契約違反: kind: instants に dates が来た——external ${src}`);
        ...
        for (let i = 1; i < pts.length; i++) {
            if (pts[i] <= pts[i - 1])
                this.err(`契約違反: 解決値は昇順・重複なし——external ${src}（§3.8 と同じ整列則）`);
        }
        for (const p of pts) {
        ...
        if (declLabels) {
            if (!Array.isArray(data.labels))
                this.err(`契約違反: labels 宣言つきの external にラベルが来ない——${src}`);
            if (data.labels.length !== pts.length)
                this.err(`契約違反: labels は時点列と同長——external ${src}（ADR-30）`);
    ...
    asofOf(members) {
        ...
        if (typeof m === 'string')
            return m;
        if (m.t === 'date')
            return this.rt.fmt(this.rt.dateToMs(m.v));
        if (m.t === 'str')
            return m.v;
        if (m.t === 'name')
            return m.name;
    ...
    registerCoverage(source, covDesc, covIv, concluded, asof) {
        const key = `${source}#${covDesc}#${asof ?? ''}`;
        if (this.rt.coverage.has(key))
            return;
    ...
    winAnnOfV(v) {
        if (isObj(v) && v.k === 'stream' && v.wins.length > 0)
            return v.wins[v.wins.length - 1].ann;
        return [];
    }
    ...
    winCovOrOut(win, ann, what) {
        const overlap = ann.filter(a => a.from < win.end && win.start < a.to);
        if (overlap.length === 0)
            return;
        const a = overlap[0];
        throw new OutOfCoverageSignal(`範囲外（out-of-coverage）: ${what}の窓 [${this.rt.fmt(Math.max(win.start, -8.64e15))}, `
            + `${this.rt.fmt(Math.min(win.end, 8.64e15))}) はマーカー覆域の外に掛かり境界が未知`
            + `（${a.source} covering ${a.covering}${a.asof ? `, asof ${a.asof}` : ''}——ADR-37 判断 4/6・F82）`, overlap, win);
    }
    /** 分類器（ADR-37 判断 6）: 失敗点が依存の実効被覆域の外＝註釈区間の内なら範囲外シグナル・
     *  内（覆域内の失敗）なら従来どおり硬エラー（取り違え＝ADR-16 の統治のまま） */
    outOrErr(ms, depAnn, hardMsg) {
        const hit = annAt(depAnn, ms);
        if (hit.length > 0) {
            const a = hit[0];
            throw new OutOfCoverageSignal(`範囲外（out-of-coverage）: 参照点 ${this.rt.fmt(ms)} は ${a.source} の実効被覆域の外`
    ...
    everyDay(env) {
        ...
        if (kindOf(day) !== 'windows')
            this.err('everyDay: 在圏暦法に day がない');
        const dw = day;
    ...
    anchorDate(d, env) {
        ...
        if (!d.hasTime)
            return tz.civilDayStart(d.y, d.mo, d.d);
        const local = Date.UTC(d.y, d.mo - 1, d.d, d.h, d.mi, Math.floor(d.s), Math.round((d.s % 1) * 1000));
    ...
    checkTzMembership(input, win, ctx, fromPredicate = false) {
        ...
        // 記録条件の精密化（還流第 17 便 §2）: ①記録するのは predicateAlign を入力に据える呼び出し元
        // だけ——fromPredicate の明示渡し（参照等値の推測は、値由来の検査〈within・単位窓×枠窓〉が
        // 偶然同一オブジェクトになる経路で誤記録し、診断の誤帰属と偽陽性を生んだ）②かつ predicateAlign
        // が束縛評価開始時（frame.baseAlign）のままの検査だけ——右辺内の filter が自分で確定した
        // align での検査は右辺内で完結し、呼び出し文脈に依存しない（再実行は過剰停止になる）
        if (fromPredicate) {
            // 重複排除（同じ検査が点ごとに積まれると、ヒット時再実行が O(N²) を戻す）——ユニークな
            // (win, ctx) は式中の射影箇所数程度なので線形走査で足りる
            for (const f of this.tzCheckLogs) {
                if (this.predicateAlign === f.baseAlign
                    && !f.log.some(c => c.ctx === ctx && gridEq(c.win, win)))
                    f.log.push({ win, ctx });
            }
        }
    ...
    calendarEntity(env, what) {
        const m = env.members.get('calendar');
        if (m === undefined)
            this.err(`${what}: 在圏に calendar: がない（前文で宣言する。§3.3/§3.9）`);
        const name = typeof m === 'string' ? m : m.t === 'name' ? m.name : null;
        if (!name)
            this.err('calendar: はカレンダー実体（premise 名）を取る（ADR-35）');
    ...
    deriveBizDay(env, entity) {
        ...
        if (nw.params.length > 0)
            this.err(`正体判定: nonWorking は引数なしの束縛（ADR-35）: ${ent.name}`);
        ...
        this.tzCheckLogs.push(tzFrame);
        try {
            const nwV = this.toStream(this.evalDef(ent, 'nonWorking', nw, env));
            // 正体判定: nonWorking は実体 tz の市民日グリッドに整列（「日粒度で読む」の操作的定義）
            const entDay = { kind: 'civil', step: 1, phase: 0, off: 0,
            ...
            const v = { k: 'stream', pts: ptDiff(every.pts, nwV.pts), wins: [], align: every.align,
                ann: annUnion(every.ann, nwV.ann), endless: true };
            this.defCache.set(cacheKey, { v, tzChecks: tzFrame.log });
            return v;
        }
        finally {
    ...
    deriveBizFine(env) {
        ...
        this.derivingEntities.add(ent.name);
        try {
            const entEnv = { ...env, premise: ent, members: entMembers };
            ...
            const entDay = { kind: 'civil', step: 1, phase: 0, off: 0, tz: entTz.name };
            ...
            const segDayStart = [];
            const segKept = [];
            const closeSet = new Set(); // 断片を閉じた実 close（bizClose の候補）
            ...
            if (since >= 0)
                push(this.rt.computeEnd); // 尾の孤立 open（切り欠き）＝実体化端まで開く
    ...
    checkAlternation(ent, sessionOpens, sessionCloses, markerAnn) {
        ...
        for (const D of domain) {
            let state = null; // 定義域の頭は不定（切り欠き許容）
            ...
            for (;;) {
                ...
                if (to === Infinity && tc === Infinity)
                    break;
                if (to === tc) {
                    i++;
                    j++;
                    continue;
                } // 同時刻対＝close→open か open→close（状態不変）
                if (to < tc) {
                    if (state === 'open') {
    ...
    openOut(ms, hit) {
        const a = hit[0];
        throw new OutOfCoverageSignal(`範囲外（out-of-coverage）: isOpen の判定点 ${this.rt.fmt(ms)} は ${a.source} の実効被覆域に依存する`
    ...
    evalList(e, env) {
        ...
        for (const el of e.elems) {
            if ('t' in el && el.t === 'range') {
                ...
                vals.push(this.evalExpr(el, env));
            }
        }
        // 時点要素のリストは時間ストリーム定数に昇格（ADR-26）
        if (vals.length > 0 && vals.every(v => isObj(v) && v.k === 'point')) {
            const pts = vals.map(p => p.ms);
            for (let i = 1; i < pts.length; i++) {
                if (pts[i] <= pts[i - 1])
                    this.err('テーブルリテラルは昇順・重複なし（§3.8 静的エラー）');
            }
            let labels;
            if (e.labels) {
                if (e.labels.t !== 'list')
                    this.err('labels: はリストを取る');
                const ls = e.labels.elems.map(el => ('t' in el && el.t === 'name') ? el.name : this.err('labels: の要素はラベル名'));
                if (ls.length !== pts.length)
                    this.err('labels: は時点列と同長（ADR-30）');
                labels = ls;
            }
            let covIv;
            let covDesc;
            ...
            const dateOnly = e.elems.every(el => ('t' in el && el.t === 'range')
                ? el.a.t === 'date' && !el.a.v.hasTime && el.b.t === 'date' && !el.b.v.hasTime
                : el.t === 'date' && !el.v.hasTime);
        ...
        if (vals.length === 0 && e.covering) {
            ...
            if (e.labels) {
                if (e.labels.t !== 'list')
                    this.err('labels: はリストを取る');
                if (e.labels.elems.length !== 0)
                    this.err('labels: は時点列と同長（ADR-30）——空テーブルに付く labels: は [] のみ');
                labels = [];
            }
            const r = this.resolveCovering(e.covering, env);
            const asof = this.asofOf(env.members);
        ...
        if (e.labels || e.covering)
            this.err('labels:/covering: は時点列（テーブルリテラル）にのみ付く');
        return vals;
    }
    /** covering-list の解決（ADR-37 判断 9）: 端は premise tz の市民日・年は暦年・
     *  端の省略＝開端（完結主張＝±∞）・区間リスト＝中抜けの申告 */
    resolveCovering(list, env) {
        const iv = [];
        const parts = [];
        const tz = this.tzObjOf(env);
        const dateStr = (ms) => {
    ...
    coveringEdge(e, env, isEnd) {
        ...
        if (isObj(v) && v.k === 'point')
            return isEnd ? tz.addCivilDays(tz.floorToDay(v.ms), 1) : v.ms;
        this.err('covering: の端は年または日付');
    }
    evalBin(e, env) {
        if (e.op === 'and')
            return this.bool(this.evalExpr(e.l, env)) && this.bool(this.evalExpr(e.r, env));
        if (e.op === 'or')
            return this.bool(this.evalExpr(e.l, env)) || this.bool(this.evalExpr(e.r, env));
    ...
    eq(l, r) {
        if (isObj(l) && l.k === 'point' && isObj(r) && r.k === 'point')
            return l.ms === r.ms;
        if (isObj(l) && l.k === 'time' && isObj(r) && r.k === 'time')
            return l.todMs === r.todMs; // ADR-51（忘れると恒偽）
        return l === r;
    }
    evalCall(e, env) {
        if (e.callee.t === 'name') {
            ...
            if (name === 'external')
                return this.evalExternal(e, env); // 外部供給宣言（ADR-46）
            ...
            if (name === 'ordinalIn') {
                ...
                this.checkTzMembership(this.predicateAlign, this.winGrainOf(wV), 'ordinalIn', true);
                const u = this.intervalsOf(uV, env);
                const w = this.intervalsOf(wV, env);
                const wi = ivIndexOf(w, d);
                if (wi < 0)
                    this.outOrErr(d, this.winAnnOfV(wV), 'ordinalIn: 点が枠窓の外');
                const ui = ivIndexOf(u, d);
                if (ui < 0)
                    this.outOrErr(d, this.winAnnOfV(uV), 'ordinalIn: 点が単位窓の外');
                ...
                let first = ui;
                while (first > 0 && u[first - 1].start >= w[wi].start)
                    first--;
            ...
            if (name === 'epochOrdinal') {
                ...
                const d = this.point(dV);
                this.checkTzMembership(this.predicateAlign, this.winGrainOf(uV), 'epochOrdinal', true); // ADR-40
                const uIv = this.intervalsOf(uV, env);
                const idx = ivIndexOf(uIv, d);
                if (idx < 0)
                    this.outOrErr(d, this.winAnnOfV(uV), 'epochOrdinal: 点が窓の外');
            ...
            if (name === 'coincides') {
                if (e.args.length !== 3)
                    this.err('coincides(S, w, d) は 3 引数（ADR-38）');
                ...
                const wi = ivIndexOf(w, d);
                if (wi < 0)
                    this.outOrErr(d, this.winAnnOfV(wV), 'coincides: 点が窓の外');
                const win = w[wi];
                ...
                if (overlap.length > 0) {
                    ...
                    throw new OutOfCoverageSignal(`範囲外（out-of-coverage）: coincides の窓 [${this.rt.fmt(Math.max(win.start, -8.64e15))}, `
                        ...
                        + `（covering ${a.covering}${a.asof ? `, asof ${a.asof}` : ''}——判定は外部・ADR-38 判断 4）`, overlap, win);
                }
            ...
            if (name === 'isOpen' && env.members.has('calendar')) {
                ...
                if (e.args.length !== 1 || e.args[0].name)
                    this.err('isOpen(t) は位置引数 1 個（ADR-41）');
                return this.isOpenAt(this.point(this.evalExpr(e.args[0].value, env)), env);
            }
            if (name === 'not')
                return !this.bool(this.evalExpr(e.args[0].value, env));
            ...
            if (local === undefined) {
                ...
                if (binding && binding.decl.params.length > 0) {
                    ...
                    // 引数付き束縛の右辺も呼び出し側 locals 鎖を遮断（ADR-53 判断 3 の完遂＝還流第 17 便 §1:
                    // この経路だけ漏れて「T(x) = dayNo(d)」が使用箇所の変数名に依存していた。実引数は遮断前の
                    // env で評価済み——値渡しの意味は不変）
                    const base = { ...(binding.premise ? { ...env, premise: binding.premise } : env),
                        locals: null, parent: null };
                    const prevSevered = this.severedLocalsEnv;
                    this.severedLocalsEnv = this.severedLocalsEnv ?? env; // 最外の遮断点を保持
                    try {
                        return this.evalExpr(binding.decl.rhs, childEnv(base, locals));
                    }
                    finally {
                        this.severedLocalsEnv = prevSevered;
                    }
    ...
    lookupBinding(name, env) {
        if (env.premise) {
            ...
            if (d)
                return { decl: d, premise: env.premise };
        }
        const cs = this.calendarSystemOf(env);
        if (cs) {
            const d = cs.findDef(name);
            if (d)
                return { decl: d, premise: cs };
        }
        const t = this.rt.topBindings.get(name);
        if (t)
            return { decl: t, premise: null };
        return null;
    }
    bindParams(params, args, env, name) {
        ...
        for (const a of args) {
            if (a.name && !keys.has(a.name))
                this.err(`${name}: 未知の名前付き引数 ${a.name}:（黙って捨てない——ADR-39）`);
        }
        if (pi < positional.length)
            this.err(`${name}: 引数が多すぎる（位置引数 ${positional.length} 個・仮引数 ${pi} 個）`);
        return locals;
    }
    ...
    applyValue(callee, args, env, calleeName) {
        if (isObj(callee)) {
            ...
            if (callee.k === 'cycle') {
                ...
                this.checkTzMembership(this.predicateAlign, callee.grain, 'cycle 射影', true); // ADR-40（weekday 等）
                const ms = this.point(args[0]);
                const idx = ivIndexOf(callee.iv, ms);
                if (idx < 0)
                    this.err('cycle: 点が対象窓の外');
            ...
            if (callee.k === 'table') {
                ...
                if (!callee.labels)
                    this.err('ラベル列（labels:）のないテーブルは射影できない');
                this.registerCoverage(callee.src ?? '(無名テーブル)', callee.covDesc, callee.covIv, callee.concluded, callee.asof);
                const ms = this.point(args[0]);
                const i = ptIndexOf(callee.pts, ms);
                if (i < 0)
                    this.outOrErr(ms, this.tableAnn(callee), 'テーブル射影: 点が列にない');
            ...
            if (callee.k === 'windows') {
                ...
                if (isValueV(args[0])) { // 値引数＝窓インスタンス参照（ADR-42 判断 4）
                    ...
                    return this.windowInstance(who, { fn: callee.labelFn }, callee.iv, callee.units, callee.grain ?? null, [], [], true, args[0], env);
                }
                if (callee.labelFn) {
                    this.checkArgIsPoint(args[0], who); // ADR-42 判断 7 (e)
                    this.checkTzMembership(this.predicateAlign, callee.grain, 'label 射影', true); // ADR-40
            ...
            if (callee.k === 'stream' && callee.wins.length > 0) {
                ...
                if (isValueV(args[0])) { // 値引数＝窓インスタンス参照（ADR-42 判断 4。segmentBy 由来の表現）
                    ...
                    return this.windowInstance(who, { fn: top.labelFn, labels: top.labels, labelsCycle: top.labelsCycle }, top.iv, callee.pts, callee.align, top.ann, callee.ann, callee.endless, args[0], env);
                }
                if (top.labelFn || top.labels || top.labelsCycle) {
                    this.checkArgIsPoint(args[0], who); // ADR-42 判断 7 (e)
                    this.checkTzMembership(this.predicateAlign, top.grain, 'ラベル射影', true); // ADR-40
                }
                if (top.labelFn)
                    return this.projectLabel(top.labelFn, top.iv, top.ann, args[0], env);
                ...
                if (top.labels || top.labelsCycle) {
                    ...
                    if (idx < 0)
                        this.outOrErr(ms, top.ann, 'ラベル射影: 点が窓の外');
                    ...
                    if (idx >= top.labels.length)
                        this.err('ラベル射影: 窓列序数がラベル列を越えた（内部整合）');
    ...
    windowInstance(who, src, iv, inputPts, align, winAnn, baseAnn, endless, v, env) {
        ...
        for (const p of inputPts) {
            while (wi < iv.length && p >= iv[wi].end)
                wi++;
            if (wi >= iv.length)
                break;
            if (p >= iv[wi].start)
        ...
        const elem = { k: 'stream', pts: elemPts, wins: [], align,
        ...
        const labelAt = (i) => {
            ...
            try {
                ...
                if (src.fn) {
                    ...
                    this.winCovOrOut(iv[i], winAnn, 'インスタンス参照');
                    if (i >= src.labels.length)
                        this.err('インスタンス参照: 窓列序数がラベル列を越えた（内部整合）');
                    label = src.labels[i];
                }
                memo.set(i, { label });
                return label;
                ...
                if (e instanceof OutOfCoverageSignal)
                    memo.set(i, { sig: e });
                throw e;
            }
        };
        ...
        return this.filterByPredicate(elem, ms => {
            const i = ivIndexOf(iv, ms);
            if (i < 0)
                return false; // 要素点列は窓所属で作るので来ない（防御）
    ...
    evalGen(e, env) {
        ...
        switch (e.word) {
            case 'grid': {
                if (kindOf(operand) !== 'chronos')
                    this.err('grid は chronos だけが受け取る（ADR-29）');
                const w = this.evalExpr(e.arg, env);
                if (!isObj(w) || w.k !== 'width')
                    this.err('grid は幅リテラルを取る');
                ...
                if (w.w.kind === 'civil') {
                    ...
                    for (let i = phase; i + stepDays < days.length; i += stepDays) {
                        ...
                        if (s >= this.rt.computeEnd)
                            break;
                        iv.push({ start: s, end: off === 0 ? days[i + stepDays] : tz.atTimeOfDay(days[i + stepDays], off) });
                    }
                    const grain = { kind: 'civil', step: stepDays, phase, off, tz: this.tzNameOf(env) };
                ...
                const iv = [];
                for (let t = t0; t < this.rt.computeEnd; t += step)
                    iv.push({ start: t, end: t + step });
                const grain = { kind: 'elapsed', step, phase: mod(t0, step), off: 0, tz: '' };
                return { k: 'windows', iv, units: iv.map(x => x.start), labelFn, grain };
            }
            case 'span': {
                ...
                if (phase < 0)
                    this.err('span: phase は 0 以上（負位相は周期を法として正規化して書く。F65）');
                const iv = [];
            ...
            case 'split': {
                ...
                if (!byName)
                    this.err('split は by: で幅の単位を明示する（§3.6）');
                ...
                parent.iv.forEach((p, y) => {
                    ...
                    const total = widths.reduce((s, w) => s + this.num(w), 0);
            ...
            case 'cycle': {
                ...
                if (!Array.isArray(labelsV))
                    this.err('cycle はラベルのリストを取る');
                const labels = labelsV.map(l => typeof l === 'string' ? l : this.err('cycle のラベルは名前'));
    ...
    genLabelFn(e, env) {
        ...
        if (labelE === undefined)
            return undefined;
        if (e.word === 'cycle')
            this.err('cycle は label: を取らない（ラベル列は cycle 自身が持つ。ADR-34）');
        return this.labelLambda(labelE, env);
    }
    labelLambda(labelE, env) {
        const v = this.evalExpr(labelE, env);
        if (!isObj(v) || v.k !== 'lambda')
            this.err(`label: はラムダ（付与式）を取る: ${kindOf(v)}（§4.9）`);
        return v;
    }
    // ---------- 本体層: core 段（§4） ----------
    ...
    applyStage(stream, stage, env) {
        ...
        switch (stage.name) {
            ...
            case 'segmentBy': {
                ...
                const covOf = (ms) => cov.find(x => x.start <= ms && ms < x.end);
                ...
                for (let i = 0; i < markers.length - 1; i++)
                    iv.push({ start: markers[i], end: markers[i + 1] });
                ...
                if (edgesV === 'error') {
                    ...
                    if (outside)
                        this.err('segmentBy(edges: error): 窓の外に点がある');
                }
                let windows = iv;
                if (edgesV === 'clip') {
                    windows = [{ start: this.rt.epoch, end: markers[0] }, ...iv];
                    if (defEnd < this.rt.computeEnd)
                        windows = [...windows, { start: defEnd, end: this.rt.computeEnd }];
                }
                if (emptiesV !== 'keep') {
                    ...
                    let wi = 0;
                    for (const p of stream.pts) {
                        while (wi < windows.length && p >= windows[wi].end)
                            wi++;
                        if (wi >= windows.length)
                            break;
                        if (p >= windows[wi].start)
                            nonEmpty.add(wi);
                    }
                    const empties_ = windows.map((_, i) => i).filter(i => !nonEmpty.has(i));
                    if (emptiesV === 'error' && empties_.length > 0)
                        this.err('segmentBy(empties: error): 空窓がある');
                    if (emptiesV === 'drop')
                        windows = windows.filter((_, i) => nonEmpty.has(i));
                }
                ...
                if (markerS.ann.length > 0 && windows.length > 0) {
                    ...
                    for (const w of windows) {
                        if (prev < w.start)
                            gaps.push({ ...like, from: prev, to: w.start });
                        prev = Math.max(prev, w.end);
                    }
                    if (gaps.length > 0)
                        winAnn = annUnion(markerS.ann, gaps);
                }
                return { k: 'stream', pts,
            ...
            case 'nth': {
                if (stream.wins.length === 0)
                    this.err(`選択子 ${stage.name} は窓なしでは型エラー（I4）`);
                let n = 1;
                if (stage.name === 'nth')
                    n = this.num(this.evalExpr(positional[0] ?? this.err('nth(n) の n が必要'), env));
                const ofE = named('of');
                ...
                for (const w of iv) {
                    ...
                    if (stage.name === 'first')
                        out.push(inWin[0]);
                    else if (stage.name === 'last')
                        out.push(inWin[inWin.length - 1]);
                    else if (n >= 1 && n <= inWin.length)
                        out.push(inWin[n - 1]);
                }
            ...
            case 'roll': {
                ...
                if (conv === 'Modified')
                    this.err('Modified は未実装（プロトタイプ）');
                if (conv !== 'Following' && conv !== 'Preceding')
                    this.err(`未知のロール規約: ${conv}`);
                const pts = [];
                for (const p of stream.pts) {
                    if (ptIndexOf(axis, p) >= 0) {
                        pts.push(p);
                        continue;
                    } // 有効点は動かない
                    if (conv === 'Following') {
                    ...
                    if (annAt(axisS.ann, p).length > 0)
                        continue; // ②実効被覆域外→空＋註釈（依存像が張る）
                    if (axis.length > 0
                ...
                if (isObj(unitV) && unitV.k === 'windows') {
                    ...
                    for (const p of stream.pts) {
                        ...
                        if (i < 0) {
                            ...
                            if (hit.length > 0) {
                                extra = annUnion(extra, hit);
                                continue;
                            } // ②範囲外→落として註釈
                            this.err('shift: 点が軸上にない（先に roll で有効点へ寄せる）');
                        }
                        const j = i + n;
                        if (j >= 0 && j < axis.length) {
                            pts.push(axis[j]);
                            continue;
                        }
            ...
            case 'rebase': {
                ...
                if (isVacuous(g))
                    return stream; // 空テーブル＝点ゼロの再錨は恒等（空虚適合。ADR-45）
                ...
                if (g.tz === toV)
                    return stream; // source == to は恒等（ADR-40 判断 5）
                const srcTz = getTz(g.tz);
                const toTz = getTz(toV);
                const mapDay = (dayStart) => {
                ...
                const mapEdge = (ms, up) => {
                    ...
                    if (up && day < ms)
                        day = srcTz.addCivilDays(day, 1); // ceil
                    for (let k = 0; k < 3; k++) {
                ...
                const align = { kind: 'civil', step: 1, phase: 0, off: 0, tz: toV };
            ...
            case 'strideBy': {
                ...
                if (wV.w.kind === 'civil') {
                    ...
                    const step = wV.w.ms;
                    for (let t = anchor; t < this.rt.computeEnd; t += step)
                        pts.push(t);
    ...
    axisMember(env, op) {
        ...
        if (!m)
            this.err(`${op}: 軸がない（on:/unit: を書くか前文で axis: を宣言。§3.3 は宣言必須）`);
    ...
    toStream(v, name) {
        if (isObj(v)) {
            ...
            if (v.k === 'point')
                return { k: 'stream', pts: [v.ms], wins: [], align: null, ann: [], endless: false };
            if (v.k === 'instant')
                this.err('everyInstant は strideBy(w, from:) と併用する（プロトタイプ）');
        }
    ...
    winGrainOf(v) {
        if (isObj(v)) {
            if (v.k === 'windows')
                return v.grain;
            if (v.k === 'stream' && v.wins.length > 0)
                return v.wins[v.wins.length - 1].grain ?? null;
        }
        return null;
    }
    ...
    intervalsOf(v, env) {
        if (isObj(v)) {
            ...
            if (v.k === 'cycle')
                return v.iv;
            if (v.k === 'stream' && v.wins.length > 0)
                return v.wins[v.wins.length - 1].iv;
        }
        this.err(`窓として使えない値: ${kindOf(v)}`);
    }
    checkPartition(iv, name) {
        for (let i = 1; i < iv.length; i++) {
            if (iv[i].start !== iv[i - 1].end) {
