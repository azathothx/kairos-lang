...
営業日ごと」「全 5 回」は選択子では書けず、これがストライドの存在意義。引数の種類と数えの使い方で三演算子に割る。
...
`stride`・`take` は**入力相対**——何を数えるかは前段が決める（「3 営業日ごと」は先に `filter(on: bizDay)`）。
...
起点（位相アンカー）は **`from:` で必ず明示**する（無ければ静的エラー。stride/take/strideBy 一族共通）。
