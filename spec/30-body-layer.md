...
- `edges:` — 最初のマーカー前／最後のマーカー後の、どの窓にも属さない点の扱い。`drop`／`clip`／`error`。
  `drop`/`error` の頭側（覆域始端〜最初のマーカー）は**窓列としては語れない**——射影・`coincides` の
  読みには範囲外註釈で応える（ADR-37 改訂 3）。
...
| 式の形 | 整列 |
|---|---|
| `everyDay`・公開窓語由来（<code>month &#124;> first</code> 等） | 要素グリッド（`Gregorian` なら `day` の grid） |
...
| `filter`・選択子・`within`・`segmentBy`・`stride`・`take`・`takeLast` | 入力の整列を保存 |
...
| <code>A &#124; B</code> | 両辺同一ならそれ、さもなくば なし（片辺が空虚適合なら**相手を継承**） |
...
## 4.7 ストライド（走査して間引く・打ち切る）

選択子とは別族の状態付きオンライン変換。選択子が窓を消費して窓相対に「第 N」を選ぶ（各窓→1 点・窓ごとリセット）の
に対し、ストライドは窓を消費せず通しで数える（境界を無視して連続・リセットしない）。「月境界を無視して N
営業日ごと」「全 5 回」「直近 3 回」は選択子では書けず、これがストライドの存在意義。引数の種類と数えの使い方で四演算子に割る。

| 演算子 | 引数 | 数えるもの | 例 |
|---|---|---|---|
| `stride(n, from:)` 入力カウント・間引く | 1 以上の整数（違反は静的エラー） | **入力ストリームの点**（軸引数なし。ADR-38・F70） | <code>filter(on: bizDay) &#124;> stride(3, from: …)</code>＝3 営業日ごと |
| `take(n, from:)` 入力カウント・**打ち切る** | 1 以上の整数（違反は静的エラー） | 入力ストリームの点（先頭 n 点のみ通す。ADR-49） | <code>(lessons \ cancelled) &#124;> take(5, from: …)</code>＝全 5 回 |
| `takeLast(n, until:)` 入力カウント・末尾で**打ち切る** | 1 以上の整数（違反は静的エラー） | 入力ストリームの点（until: 以前〈含む〉の末尾 n 点のみ通す。ADR-52） | <code>tensha &#124;> takeLast(3, until: …)</code>＝直近 3 発火 |
| `strideBy(w, from:)` 幅刻み | 幅＝複数軸の物理量 | 幅（絶対量） | `strideBy(24h39m35.244s, from: …)`＝1 sol ごと |

`stride`・`take`・`takeLast` は**入力相対**——何を数えるかは前段が決める（「3 営業日ごと」は先に `filter(on: bizDay)`）。
...
`take` は RRULE の COUNT 相当だが、**除外後に数える**が合成順で出る——`(lessons \ cancelled) |>
take(5, from: …)` は除外で減った分だけ後続点が繰り上がって n 個に達する（COUNT が除外前の生成
集合を数えて総回数が痩せる轍を、構造で踏まない）。第 n 点より後は**正当な空**（註釈なし・ADR-37
判断 2）。**窓付き入力は誘導つき静的エラー**——「窓ごとの先頭 N」は within の後の nth（§4.3）か
`ordinalIn` の述語（§4.9）で書く（take は通し数え）。期間限定は評価範囲の分離が正道で take の
守備ではない。

錨（位相アンカー）は**必ず明示**する（無ければ静的エラー。一族共通——stride/take/strideBy は
起点 `from:`・takeLast は終端 `until:`〈from: の双対・取り違えは専用診断〉。ADR-52）。
...
**展開は有限——定義の循環（自己・相互再帰）は誘導つき静的エラー**（F110。正式な規定は §5.3——
「全糖衣を展開すれば core だけが残る」の前提そのもので、再帰は書けない）。
...
値式（ラムダ）の中で使い、`filter` と組んで「窓の座標で選ぶ」式を作る。読む側の中核は次の三語。

| 語 | 型 | 意味 |
|---|---|---|
| `ordinalIn(u, w, d)` | 点 → 数値 | 点 `d` が属する `w` 窓の中で、`d` が属する `u` 窓が第何番目か（1 起点）。`ordinalIn(day, month, d)`＝月内の第何日。数える単位 `u` を明示するので入力粒度に依存しない。**整合検査**（ADR-50）: u が経過幅グリッドのとき、d の属する w 窓の開始が u の目盛り上にないと明示エラー（紀元差が単位幅の整数倍でない tz——黙って半端な序数を出さない。per-instance・データ相対の層） |
...
| `coincides(S, w, d)` | 点 → 論理値 | 点 `d` の属する `w` 窓の中に、ストリーム `S` の点が少なくとも一つ在るか（**窓所属の述語**＝値式の有界存在量化。ADR-38・F68）。一族で唯一、第 1 引数がストリーム（窓語は静的エラー——点列は <code>month &#124;> first</code> で） |
...
| take | stride 行の同型＋**縮小**: 第 n 点が交差より前に確定したら、確定後に始まる註釈区間は輸送しない（出力は入力の先頭 n 点にしか依存しない——「n 到達後は正当な空・註釈なし」との整合の要件。ADR-49） |
| takeLast | take の**鏡像**: 逆向き数えが註釈区間に交差したら交差点から**過去側すべて**（head 拡幅 (-∞, …]）＋**条件付き縮小**: 逆向き数えが交差せず第 n 点（過去端）が確定した場合に限り、それより過去で完結する註釈区間は輸送しない（出力は [第 n 点, until] の入力にしか依存しない。ADR-52 判断 4） |
| segmentBy | マーカー覆域の補集合。`edges:`/`empties:` の発火は**覆域の端**（列の端ではない）——覆域内は最終マーカー起点の窓も覆域端まで確定（＝窓列の**実効被覆域**）。窓の張られない区間（`edges: drop`/`error` の頭側・`empties: drop` の中抜け）は窓列の範囲外＝註釈（ADR-37 改訂 3） |
...
表の像を一枚に並べる——covering の中抜け〔2027-01-20, 2027-05-10〕に依存する入力 S（範囲外註釈がその
区間に立つ）を、評価範囲 [2026-07-01, 2028-01-01) で各演算子に通した出力の註釈区間。shift は
`shift(+1, unit: month)`、選択子と filter は月窓を読む形（`first(of: month)`・coincides 級の述語）。
結合子の和・roll の依存像は表のとおり:
...
要点: stride だけ右に閉じない（中抜けの先でデータが戻っても、歩行の位相は既に汚染されている）。
選択子と filter の像が一致するのは、この例がどちらも同じ月窓を読むため。
