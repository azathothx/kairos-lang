...
| 記号 | 層／役 | 意味 |
|---|---|---|
| <code>&#124;></code> | 両層 | 段の連結（本体層: stream → stream ／ premise 層: premise → premise） |
...
- ストライド（選択子と別族）: `stride(n, from:)` 入力カウント（**入力の点**を n ごと・軸引数なし・
  n は 1 以上の整数＝ADR-38）／`take(n, from:)` 先頭 N 選択（入力の点の先頭 n 点のみ・窓付き入力は
  静的エラー＝ADR-49）／`takeLast(n, until:)` 末尾 N 選択（until: 以前〈含む〉の入力の点の末尾
  n 点のみ・窓付き入力は静的エラー＝ADR-52）／`strideBy(w, from:)` 幅刻み（複数軸の物理量）。
  錨は必須（ADR-31——stride/take/strideBy は起点 `from:`〈数え起点は from: 以上の最初の入力点〉・
  takeLast は終端 `until:`〈第 0 歩は until: 以下の最後の入力点〉）、リセットは既定しない。
...
- `|> rephase(δ, on: W, unit: U) : premise -> premise` — 窓 `W` の切れ目を単位 `U` で δ ずらす糖衣。
...
- 宣言印なし（糖衣性は依存解析で自動判定、core 語再定義・**定義の循環**〈自己・相互再帰。展開の
  有限性の帰結〉は静的エラー）。premise は焼き込まず遅延解決。展開＝右辺の
  機械的差し込み（core への片方向）。網羅解説と実行例は `../reference/sugar-definition.md`。
...
**確定（2026-07-26・設計者裁定＝最後の仮称の解消）**: 窓境界の位相ずらし糖衣は **`rephase`**
（旧仮称 `shiftBoundary`）。展開の実体（`span` の `phase:` 差し替え）と名が一致し、`rebase`（再錨）・
`anchor:`（位相を留める実日）と語彙系で対称。比較候補 `shiftBoundary` 昇格（点変換 `shift` と別物
なのに同語幹・13 字）・`shiftPhase`（同語幹の紛れが残る）・`offsetBoundary`/`moveBoundary`（新語幹）
は不採用。`boundary` の語は `k` 可変組を扱う将来の別演算子（§3.7・スコープ外）に温存する。
**これで全語彙が正式名——仮称は残っていない**（経緯は CHANGELOG 追補 11）。
追記（2026-08-17・ADR-49/50）: 後発の **`take`**（ストライド族の第 3 語）は名の候補比較
（`first(n, from:)`＝窓選択子 first と衝突・不可）を経て確定。premise 公開語 **`hour`**（ADR-50）
は語彙表の外（公開語は 1.0 一括確定の流儀）。
追記（2026-08-21・ADR-51/52）: **`takeLast`**（ストライド族の第 4 語・錨 `until:`）は候補比較
（`take(n, until:)`＝同語 named-arg での意味反転・`last(n,…)`＝選択子衝突・`recent`＝「今」を
内蔵する響き——いずれも不採）を経て確定。stdlib 糖衣 **`at`**（ADR-51）は公開語側
（`dailyAt`＝生成子誤読・`atTime`＝T 接頭と二重表現で不採）。
...
- **単独時刻リテラル**（ADR-51）: `Thh:mm(:ss(.f+)?)?`——日付部を持たない壁時計時刻。値は日内
  時刻で、`strideBy(1d, from:)` の位置でのみ時点に具現する（epoch 錨日 1970-01-01・在圏 tz の
  錨打ち＝壁時計 tick の起点。stdlib 糖衣 `at` の引数がこの字句の主用途——**引数はこの字句に
  限る**〈日付つきの錨は前方専用の欠落・窓付き入力は窓の脱落——ともに誘導つき静的エラー。
  ADR-51 追記〉。他の点位置・他の幅は
  誘導つき静的エラー——黙って epoch 日に固定しない）。時刻部の桁域は日付リテラルと同一。
  **T 接頭形限定**——裸 `hh:mm` は三項条件の合法式（`cond ? 10:30`）と衝突するため字句にしない。
  `T\d\d:` に続く不正形は誘導つき字句エラー（黙って識別子に落とさない）。
...
- **数値**: 整数・小数。**識別子・列挙ラベル**: Unicode 文字を許す（`甲`・`子` 等の漢字ラベル可）。予約記号
  （`|> . | & \ = => < > [ ] ( ) { } : , ? @ #`）と空白は識別子に使えない。ラベル値の等値は**字面の一致**——
  識別子ラベルと同字面の文字列リテラル（`立春` と `"立春"`）は等値。ただし**名前を語彙に導入するのは
  ラベル位置の識別子宣言だけ**で、文字列だけで宣言したラベルを裸の識別子で参照すると「未解決の名前」に
  なる。ラベルの宣言は識別子形が推奨（文字列リテラルの本来の役割は `tz:`・`source:` の値＝ADR-32）。
- **コメント**: `#` から行末まで。
...
構造・語彙とも確定（§5.4——仮称は残っていない）。糖衣（`a..b` 範囲、ポイントフリー略記）は
展開で core に消える。
...
本仕様の範囲では、`k` 可変組の射程外を扱う別演算子（必要になれば導入・`boundary` の語を温存）・
`of:` の匿名窓ラベル付けが未確定として残る（命名は**全語確定**＝`rephase` の裁定〈2026-07-26・
§5.4〉で仮称ゼロ・外部供給宣言は **`external` として確定**＝ADR-46・§3.8）。いずれも意味論を変えない
（追加拡張。DoD の分類は `../design/90-open-questions.md` 冒頭）。
