...
（補記 2026-08-26・第 11 回レビュー G の補綴: 本追補の前日 2026-08-20 に **F110＝糖衣定義の
循環検出**〈自己・相互再帰を経路つき静的エラーへ——従来は JS 生エラーで落ちていた挙動変更を含む〉
と **reference「糖衣定義」ページ新設**〈doctest 3 本〉・spec §4.8 の循環明文化が入っている。
追補相当の変更だが番号を持たず、記録がこの欄から漏れていた。）
...
還流第 15 便（供給側の追随実装知見 5 件＝収蔵 16・全件を参照実装で再現確認）が駆動。at の展開形は
入力を**述語として読み直す**ため、入力が運んでいた窓・覆域註釈のかかった点・endless の由来が
出力へ引き継がれない——うち「利用者が黙って誤結果を得る」2 形を**展開前の名指し検査**で塞ぐ
（§5.5・stdlib §2）: **(a) 窓付き入力は誘導つき静的エラー**（ADR-49 の窓ガードが 1 段で素通り
し通し数えに化ける形。正道＝窓は at の後で切る——外延同値）・**(c) 引数は単独時刻リテラルに限る**
（日付つきの錨〈日時・裸日付とも〉は前方専用で錨より前が黙って空）。検査は標準語彙の名の防御で、
利用側 premise の同名再定義には掛からない（糖衣原理 ADR-23 の例外ではなく展開前の適格性検査）。
覆域外の点の at 段での脱落と endless 由来の警告偽陽性は**規約の内**（糖衣は展開先の意味を継承）
——reference の落とし穴へ実測込みで文書化。判断 6 の括弧書き（tick 生成域）は実装事実に合わせ
追記で訂正——horizon-clip が出ない論拠は「単位窓列を実体化しない」の側。607 テスト。
...
②`width-literal` を実装の受理形へ（旧 `elapsed-width` は**空文字列を導出でき**・順序固定が実装
〈セグメント 1 回以上の列〉より狭かった——`width-seg` の 1 回以上繰り返しに統一・civil/elapsed の
分類は検査の層と明記）③束縛後置の `covering:` を**引数なし束縛の分岐に限定**（実装の静的エラー
...
受理言語・意味論・診断文言とも**不変**（テスト側のみ）。供給側の変異試験が検出した 3 件——
...
## 1.0（2026-09-14）——RC5 から 1.0 へ

受理言語・意味論・演算子族・文法（EBNF）・字句を **1.0 として凍結**する。RC5（2026-07-08 宣言）から
追補 18（2026-08-31）までの精密化はすべて「受理言語を変えない」制約の下で行われ、本節はそれを版として
封じる宣言である。参照実装は npm `kairos-lang` 1.0.0（プロトタイプ・実行時依存ゼロ）。以後の変更は
「1.0 追補 N」として本ファイルに追記する（状態行の追補番号検査は同じ網で読み替える）。

## 1.0 追補 19（2026-09-24）——派生 premise の前文メンバー継承・`roll:` メンバー・正準例の改名（還流 2026-09-23 の 2 便）

受理言語・字句・文法は**不変**。①**派生 premise の前文メンバー継承**（ADR-35 改訂 3・spec §3.7 に明文）——`premise Mine = Base
with { … }` を `@` の入口にしたとき、本体層が直接引く `calendar:`（`bizDay` の標準導出）・`axis:`・**`tz:`** が Base の宣言を見ず、
tz は黙って機械の tz に落ちていた（公開語の内側は判断 8 の上書き重ねで見えており、同じ式が置き場所で tz を変える非対称。供給側
の照会を再現）。登録時に基底連鎖のメンバーを継承しブロックの宣言が上書きする形へ——入口の本体層が見る集合を公開語の内側と同じに
する（`source:` も同じ集合＝従来の overlay どおり。判断 5 の「必須寄り」は未執行のまま）。②**前文メンバー `roll:`**（§3.3 の規約の
畳み込み）を実装——規約の位置引数が無いとき在圏の `roll:` に落ちる（明示推奨の但し書きは不変）。③`reference/segmentBy.md` の
正準例の補助関数名 `monthNo` → **`monthLabelNo`**（標準 Gregorian の `monthNo` と同名衝突し、`with` 派生では機構 A の再解決で
標準 `month` のラベル射影まで差し替わって月番号の読み出しが既定枝の値に潰れる・警告 0＝供給側の緊急便の訂正。註記と `label:` 形の
長さ検査の落とし穴を日英に追加）。標準語の再定義への警告は**言語側では採らない**（90-open-questions に明示の却下）。witness 9 本
追加＝**657 テスト**（修正を落とすと 6 本が赤＝変異試験済み）。参照実装 npm `kairos-lang` **1.0.2**。
