...
[English](./) | **日本語**（ドキュメントは日本語が正・spec/reference/stdlib は英語フルミラーあり〈旧暦のみ日本語〉）
...
<p align="center"><img src="assets/figures/hero-two-layers.ja.svg" width="880"
  alt="二層構造: premise 層がカレンダーを定義（内包）し、評価は一方向。本体層がスケジュールを紡ぎ、出力の時間ストリームはさらに次の定義へ流せる（閉包）。"></p>
...
**ステータス: リリース候補（RC5＝2026-07-08 宣言・追補 15〈2026-08-26〉まで反映）**。意味論・演算子族・文法（EBNF）・字句は確定、命名も
**全語確定**（最後の仮称 `shiftBoundary` は `rephase` に裁定済み＝2026-07-26）。表現力は既知スケジュール 20 要素のサンプル
検証と、リファレンス実装での実行検証（国立天文台 暦要項の実データ照合を含む・618 テスト）で実証済み。
...
| [`recipes/`](recipes/) | **レシピ**——cron/Quartz/RRULE で「書けない」とされてきた要求の実用形（1 要求 1 ページ・Playground リンクつき） |
...
| [`design/`](design/) | 設計記録（ADR-01〜53・ドメインモデル・構文ドラフト・表現力検証〈[「書けないスケジュール」の博物誌](design/40-examples/11-impossible-schedules.md) 含む〉・綻びログ） |
...
- 「Kairos」の名称・ロゴ（[assets/logo/](assets/logo/README.md)）はライセンスの対象外です（Apache-2.0 §6）。
