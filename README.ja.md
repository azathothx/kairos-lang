...
**ステータス: リリース候補（RC5＝2026-07-08 宣言・追補 8〈2026-07-14〉まで反映）**。意味論・演算子族・文法（EBNF）・字句は確定、命名も
一括確定済み（仮称は `shiftBoundary` 一語＝1.0 で確定）。表現力は既知スケジュール 20 要素のサンプル
検証と、リファレンス実装での実行検証（国立天文台 暦要項の実データ照合を含む・450 テスト）で実証済み。
...
| [`design/`](design/) | 設計記録（ADR-01〜46・ドメインモデル・構文ドラフト・表現力検証・綻びログ） |
