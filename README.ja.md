...
**ステータス: リリース候補（RC5＝2026-07-08 宣言・追補 13〈2026-08-21〉まで反映）**。意味論・演算子族・文法（EBNF）・字句は確定、命名も
**全語確定**（最後の仮称 `shiftBoundary` は `rephase` に裁定済み＝2026-07-26）。表現力は既知スケジュール 20 要素のサンプル
検証と、リファレンス実装での実行検証（国立天文台 暦要項の実データ照合を含む・603 テスト）で実証済み。
...
| [`design/`](design/) | 設計記録（ADR-01〜52・ドメインモデル・構文ドラフト・表現力検証・綻びログ） |
