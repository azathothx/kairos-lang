...
- 実行の都合（F80・執筆時）: 当時のプロトタイプは本体層の束縛を defCache せず、値述語から参照される
  束縛（`band` 等）を本体層に置くと点ごとに再評価された（実測 1 秒 → 50 秒）——premise 側に置く
  誘導はこのため。**2026-08-03 に本体層束縛もメモ化され解消**（impl/README。例は premise 束縛の
  まま＝供給規約の整理としては変わらず自然）。
