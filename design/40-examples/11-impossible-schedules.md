...
| 29 | 「除外が出ても合計 n 回」（講座 5 回・キャンセル補充） | COUNT は除外**前**に数える・[rrule.js でも不可](https://github.com/jkbrzt/rrule/issues/456) | ○語彙（`take(n, from:)`＝ADR-49。初版判定は**射程外**——受け皿明文化が需要証拠となり導入へ→ §11.5） |
...
`hour` 窓は実測当時の Gregorian 標準に無かった（本節の実測が需要根拠となり **ADR-50 で標準化**
＝§11.5。以下は当時の書き方のまま残す——**暦の原子は 1 行で張れる**〈grid・ADR-41 の第二用途〉
の実演として今も有効）:
...
**(A) 語彙で書ける**——カタログ 32 項目中 23 項目（初版 22——カタログ 29 が ADR-49 `take` で
語彙へ移動＝§11.5）。挫折の根は既存方式の**構造**（フィールドの
...
**(C) 設計上の射程外**——受け皿を明文化して断る型。初版収集では 3 つが落ち、うち「先頭 N 個」は
ADR-49 で語彙へ移動（§11.5）——**現在は 2 つ**:
...
| ~~「先頭 N 個」の選択（COUNT 型・除外後の補充）~~ | ~~評価範囲・covering での有界化~~ → **ADR-49 `take(n, from:)` で語彙へ移動** | **実需要の初の証拠**（rrule.js #456・講座ビジネスの定番）→ §11.5 |

これらに共通するのは、**受け皿が「黙って近似する」でなく「分解の仕方を明文化する」**である
...
555 テスト
（注記 2026-08-28＝第 11 回レビュー LOW の補綴: 追補 12〈同日 2026-08-17〉の「552」との差 3 本は
採取時点の差——本文書の 555 は同日中の後続コミット〈ADR-49 の追加分〉込みの値。矛盾ではなく
採取順の記録として残す）。

## 11.6 まとめ——1.0 訴求材料として

- **「書けないと言われてきたもの」の実測 17 本・全て現行語彙・言語変更ゼロ**。収集 32 項目の
  判定は A 23・B 7・C 2（初版 A 22・C 3——§11.5 の ADR-49 `take` 導入でカタログ 29 が移動）
  ——射程外はすべて受け皿明文化済み。
