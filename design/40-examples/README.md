...
本ディレクトリは RC 期の**探索の記録**であり、ファイルにより検証の形態が異なる。「書ける」の
機械的な裏づけがどこにあるかの対応:

| ファイル | 本文コード例の検証 | 機械検査の所在 |
|---|---|---|
| 01 祝日一式 | 期待値照合（記述当時は実装前） | 中核形は実装テストが実行検証——祝日カスケード（振替 5/6・国民の休日 9/22 の導出）と給料日は `impl/test/examples.test.ts`（spec §7.4/7.5）、営業日導出は doctest 標準前提そのもの |
...
| 05〜11 | **doctest（生きたテスト）**——`# eval:` 付きフェンスを `impl/test/doctest.test.ts` が走査・実行 | 本文がそのままテスト（ティティ・営業時間・注入起点・バックアップ多段・DST と幅規約・代替暦の算術形・「書けない」実例の実測 17 本） |
...
| ファイル | 対象 |
|---|---|
| [01-jp-holidays.md](01-jp-holidays.md) | 国民の祝日一式（固定日・ハッピーマンデー・春分秋分・振替・国民の休日・五輪特例）＋生活系（給料日・ゴミ収集・中国本土の調休） |
...
| [90-findings.md](90-findings.md) | 綻びログの集約（F1〜F111）と補完機構への写像 |
...
| 中国本土の調休（反転） | 書ける | テーブルリテラル・カスケード差 |
...
| 「1 日後」vs「24 時間後」（DST 切替を跨ぐ） | 書ける | 幅の二リテラル `1d`/`24h`＝混同が書けない（09） |
